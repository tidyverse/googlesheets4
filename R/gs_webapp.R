...
#'
#' Build the Google URL that googlesheets needs to direct users to in order to
...
#' `redirect_uri` (see below) which [gs_webapp_get_token()] uses to exchange for
#' an access token. This token is then stored in the usual manner for this
#' package and used for subsequent API requests.
#'
#' That was the good news. The bad news is you'll need to use the [Google
#' Developers Console](https://console.developers.google.com) to _obtain your
#' own client ID and secret and declare the `redirect_uri` specific to your
#' project_. Inform googlesheets of this information by providing as function
#' arguments or by defining these options. For example, you can put lines like
#' this into a Project-specific `.Rprofile` file:
#'
#' ```
...
#' ```
#'
#' Based on Google Developers' guide to [Using OAuth2.0 for Web Server
...
#' [gs_webapp_auth_url()] to get an access_token. This function plays
#' a role similar to [gs_auth()], but in a Shiny-based workflow: it
...
#' for [gs_webapp_auth_url()] for more details on OAuth2 within Shiny.
#'
