...
Basic Sheet writing is shown in the [Get started](https://googlesheets4.tidyverse.org/articles/googlesheets4.html) article. Here we explore the different uses for the writing functions:
  ...
  * `sheets_flood()` writes common content into all of the cells in a range.
...
`sheets_flood()` "floods" all cells in a range with common content. By default, it floods them with no content, i.e. it clears cells of their existing value and format. Note that `sheets_clear()` is a convenience wrapper for this special case. 
First, we create a data frame and initialize a new Sheet with that data.

```{r}
df <- sheets_fodder(10)
(ss <- sheets_create("sheets-flood-demo", sheets = list(df)))
```
...
By default, `sheets_flood()` sends an empty cell (and clears existing formatting).
...
`sheets_sheet_resize()`. If you just want to clear values or formats, use `sheets_clear()`.
