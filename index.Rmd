...
`sheets_create()` creates an entirely new Sheet and can, optionally, populate individual (work)sheets from a named list of data frames. Remember that `sheets_browse()` takes you to a Sheet in the browser.
...
`sheets_write()`, also aliased to `write_sheet()`, writes a data frame into an existing (work)sheet within an existing (spread)Sheet *this is likely to get more flexible*`:
