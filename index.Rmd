...
`sheets_create()` creates an entirely new Sheet and can, optionally, populate individual (work)sheets from one or more data frames. Remember that `sheets_browse()` takes you to a Sheet in the browser.
...
`sheets_sheet_add()` adds a worksheet and, optionally, lets you specify where to put it. It's part of a family of `sheets_sheet_*()` functions that deal with individual (work)sheets inside a (spread)Sheet.
...
`sheets_write()`, also aliased to `write_sheet()`, writes a data frame into a
(work)sheet (possible pre-existing) within an existing (spread)Sheet *this is likely to get more flexible*. Here we send the `warpbreaks` data to the empty sheet we created above:
