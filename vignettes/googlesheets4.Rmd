...
The metadata exposed by `gs4_get()` is also revealed whenever you print an object that is known to be a `sheets_id` (an S3 class we use to make Sheet IDs).
...
`range_write()` writes a data frame into a Sheet. The only required argument is the data.
...
ss <- gs4_create("fluffy-bunny", sheets = df)
ss
```

`range_write()` can write to new or existing (work)sheets in this Sheet. Let's write the `chickwts` data to a new sheet in `ss`.

```{r}
range_write(chickwts, ss)
ss
```

We can also use `range_write()` to replace the data in an existing sheet.

```{r}
range_write(data.frame(x = 4:10, letters[4:10]), ss, sheet = "df")
ss
```
...
`range_write()` makes it possible to write arbitrary data, into an arbitrary range. It has a very different "feel" from `gs4_create()`, `sheet_write()`, and `sheet_append()`, all of which assume we're writing or growing a table of data in a (work)sheet. `range_write()` makes fewer assumptions about what it's writing and why.
