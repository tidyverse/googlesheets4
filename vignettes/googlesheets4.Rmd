...
```{r}
ss %>% sheets_append(data.frame(x = 11, letters[11]), sheet = "df")
ss
```

`range_write()` makes it possible to write arbitrary data, into an arbitrary range. It has a very different "feel" from `sheets_create()`, `sheets_write()`, and `sheets_append()`, all of which assume we're writing or growing a table of data in a (work)sheet. `range_write()` make fewer assumptions about what it's writing and why.
