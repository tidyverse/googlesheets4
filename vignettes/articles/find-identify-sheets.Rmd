...
googlesheets4 accepts such a URL as the `ss` (think "spreadsheet") argument of many functions:
...
At this time, although the URL specifies both a (spread)Sheet and a (work)sheet, we only extract the Sheet ID.
...
Note this demonstration that the Sheet ID also works when provided as a plain, old string, i.e. it is does not **have** to have the `sheets_id` class.
...
Since we generally call `as-sheets_id()` on whatever the user provides as `ss`, you can even pass `gap_dribble` straight into googlesheets4 functions.
...
Two important things to note:
  ...
  * Auth: googledrive requires auth for functions like `drive_get()` and
...
See the examples for `googledrive::drive_find()` and `sheets_find()` for more ideas about how to search Drive effectively for your Sheets.
