...
Let's say you have a cheerful Google Sheet named "deaths". If you want to access it by name, use googledrive to identify the document (capture its metadata, especially file id). Pass the result to functions like `sheets_get()` (gets file metadata) or `read_sheet()` (gets cell data).
