...
Several googlesheets4 functions allow the user to specify which cells to read or write. This range specification comes via the `sheet`, `range`, and (possibly) `skip` arguments. Examples of functions with this interface: `sheets_read()`, `range_read_cells()`, and `range_write()`.
  ...
  * `skip` is an optional argument that appears in `sheets_read()` and 
    `range_read_cells()`, to be compatible with functions like `read.table()`,
...
`range_write()` is the only writing function in googlesheets4 that lets the user target a specific range. This function is why we need the ability to convert A1-notation to `cell_limits` and to decide whether user's range should be sent via `start` or `range`. `sheets_write()`, `sheets_create()`, and `sheet_append()` all offer a higher-level API, focused on the holistic management of (work)sheets that hold a single data table.
