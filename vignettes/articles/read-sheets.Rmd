...
```{r}
range_read(sheets_example("mini-gap"), sheet = 2)

range_read(sheets_example("mini-gap"), sheet = "Oceania", n_max = 3)

range_read(sheets_example("deaths"), skip = 4, n_max = 10)
...
system.time(
  sheets_example("gapminder") %>% range_read(sheet = "Africa")
)
```

The modest difference above shows that the speed difference is unlikely to be a gamechanger in many settings.

Summary of how to think about `sheets_speedread()` vs `range_read()`:
